...
export const EvidenceDisplay: React.FC<EvidenceDisplayProps> = ({ evidences, maxLength = 200 }) => {
  useEffect(() => {
    // eslint-disable-next-line no-console
    console.log('Evidences ricevute in EvidenceDisplay:', evidences);
  }, [evidences]);
    ...
    switch (evidence.type) {
      ...
      case 'paragraph_pair':
        if ('similarity' in evidence) {
          const aiLikelihood = 1 - evidence.similarity;
          return (
            <div className="evidence-content-simplified">
              <div className="evidence-text-main">
              ...
              <div className={`evidence-score-prominent ${getScoreLevel(aiLikelihood)}`}>
                {Math.round(aiLikelihood * 100)}%
              </div>
            </div>
          );
        }
